...
import { comparePassword } from './argon2'
...
import * as JuniorServiceModule from '../junior/junior.service'
import type { JuniorService } from '../junior/junior.service'
...
export class AuthenticationService {
    ...
    constructor(
        ...
        @Inject(forwardRef(() => JuniorServiceModule.JuniorService))
    ...
    // In the attempt the provided password should be in plaintext, but expected password as a PHC string.
    private async validateYouthWorker(attempt: { provided: string, expected: string }, userId: string): Promise<void> {
        const passwordMatch = await comparePassword(attempt.provided, attempt.expected)
        if (!passwordMatch) {
