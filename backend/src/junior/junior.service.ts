...
export class JuniorService {
    ...
    async registerJunior(registrationData: RegisterJuniorDto, userId?: string, noSMS: boolean = false): Promise<string> {
        ...
        if (existingJunior) {
            ...
            if ([STATUS.EXPIRED, STATUS.PENDING].includes(existingJunior.status)) {
    ...
    async createNewSeason({ expireDate }: SeasonExpiredDto, userId?: string): Promise<string> {
        ...
        const result: UpdateResult = await this.juniorRepo.createQueryBuilder().update().set({ status: STATUS.EXPIRED }).execute()
