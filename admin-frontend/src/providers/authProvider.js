...
export const authProvider = (type, params) => {
    if (type === AUTH_LOGIN) {
        ...
        return new Promise((_, reject) => {
            httpClient(url, options).then(response => {
                ...
                localStorage.setItem(userTokenKey, response.access_token);
    ...
    if (type === AUTH_CHECK) {
        return localStorage.getItem(userTokenKey) ? Promise.resolve() : Promise.reject();
    }
    if (type === AUTH_LOGOUT) {
        ...
        if (auth_token) localStorage.setItem(userTokenKey, auth_token);
